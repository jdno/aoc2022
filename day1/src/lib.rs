...
pub fn items_per_elf(input: &str) -> Vec<Vec<usize>> {
    let mut items_by_elf = Vec::new();
    ...
    for line in input.lines() {
        ...
        if sanitized_line.is_empty() {
            if !items.is_empty() {
                items_by_elf.push(items);
            }
            items = Vec::new();
        } else {
            let calories = sanitized_line
                .parse::<usize>()
...
pub fn calories_per_elf(items: &[Vec<usize>]) -> Vec<usize> {
    items
        .iter()
        .map(|items_by_elf| items_by_elf.iter().sum())
        .collect()
}

pub fn maximum_calories(calories: &[usize]) -> usize {
    *calories
        .iter()
        .max()
        .expect("failed to find maximum calories")
}

pub fn top_three(calories: &mut [usize]) -> usize {
    calories.sort();
...
mod tests {
    ...
    #[test]
    fn items_per_elf_parses_input() {
        let result = items_per_elf(EXAMPLE);
    ...
    #[test]
    fn calories_per_elf_reduces_items() {
        let input = vec![vec![1000, 2000, 3000], vec![4000]];

        let result = calories_per_elf(&input);

        assert_eq!(result, vec!(6000, 4000));
    }

    #[test]
    fn maximum_calories_returns_largest_number() {
        let input = vec![6000, 4000];

        let result = maximum_calories(&input);

        assert_eq!(6000, result);
    }

    #[test]
    fn top_three_returns_sum() {
        let mut input = vec![1000, 4000, 2000, 3000];

        let result = top_three(&mut input);
